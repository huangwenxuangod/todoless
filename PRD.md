...
> 版本: v0.3.0
> 日期: 2026-05-09
> 状态: 多端 workspace 已建立；桌面端为 MVP 主线，移动端与 Web 端已开始实现
...
- 移动端已接入 Expo Router、Expo SQLite、Zustand，并复用 shared 的任务类型/日期逻辑。
...
- **Sync**: 预留 Supabase 方向；当前移动端仍以本地 SQLite 为主
- **Styling**: StyleSheet + 共享 design tokens

### 4.2 设计适配原则

- **视觉统一**: 颜色、字体、圆角、间距与桌面端完全一致 (via `tokens.ts`)
- **布局适配**: 移动端任务页单列；设置页在窄屏也保持左右结构，左侧仅展示图标，右侧内容滚动
