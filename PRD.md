...
> 版本: v0.3.2
> 日期: 2026-05-13
> 状态: 桌面端继续作为 MVP 主线；移动端已具备本地任务、语音录入、通知提醒与同步服务骨架；Web 已具备 Resend waitlist
...
- 桌面端支持 OpenRouter 文本规划，默认模型 `moonshotai/kimi-k2.6`，并在失败时回退到 `qwen/qwen3.6-flash`、`deepseek/deepseek-v4-flash`。
...
- 移动端已实现按住说话的 OpenRouter 语音链路、移动端本地任务命令执行、Expo Notifications 提醒调度，以及 Done / Later 通知动作。
...
- Supabase 初始同步迁移与桌面/移动服务 helper 已建立，第一版同步范围为 `tasks / tags / task_tags / sync_state`，暂不同步 `events / recent_context / AI 偏好记忆`。完整登录 UI、首轮上传/拉取编排和冲突 UX 仍不属于当前 MVP 表面。
...
- **Sync**: Supabase service helper 已存在；当前移动端仍以本地 SQLite 为主，登录/自动同步编排尚未接入产品流
...
2. **服务 helper 已有**: 桌面端通过 Supabase REST helper，移动端通过 `@supabase/supabase-js` helper 拉取/推送 `tasks / tags / task_tags`。
3. **增量同步目标**: 只传输 `updated_at > last_synced_at` 的记录。
4. **冲突解决目标**: Last-Write-Wins (LWW)，以 `updated_at` 时间戳为准。
5. **当前缺口**: 登录 UI、首次本地数据上传、后台调度、离线队列和冲突提示尚未接入产品流。

#### 认证

- **方式**: Supabase Auth email magic link。
- **理由**: 与跨设备同步目标一致，避免在 MVP 里引入复杂账号密码管理。
...
| `sync/index.ts` | Supabase 同步协议行/载荷类型 | 已有类型定义 |
